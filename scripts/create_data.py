...
def main():
    ...
    testdata = datasets.TestData(dataset_root, iscrop=True, size=args.
        image_size, sort=True)
    ...
    if args.use_meanshape:
        ...
        mean_shape = paddle.mean(x=paddle.concat(x=shapes, axis=0), axis=0,
            keepdim=True)
    ...
    with lmdb.open(output_dir, map_size=1024 ** 4, readahead=False) as env:
        ...
        for batch_id, data in enumerate(tqdm(loader)):
            with paddle.no_grad():
                ...
                if args.use_meanshape:
                    codedict['shape'] = mean_shape.tile(repeat_times=[inp.
                        shape[0], 1])
                codedict['tform'] = tform
                opdict, _ = deca.decode(codedict, render_orig=True,
                ...
                for item_id in range(inp.shape[0]):
                    ...
                    total += 1
        with env.begin(write=True) as transaction:
            transaction.put('length'.encode('utf-8'), str(total).encode(
                'utf-8'))


def create_argparser():
    defaults = dict(data_dir='', output_dir='', image_size=256, batch_size=
        8, use_meanshape=False)
