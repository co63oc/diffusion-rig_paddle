...
def image_align(src_file, dst_file, face_landmarks, output_size=1024,
    ...
    crop = max(crop[0] - border, 0), max(crop[1] - border, 0), min(crop[2] +
        border, img.size[0]), min(crop[3] + border, img.size[1])
...
class LandmarksDetector:
    ...
    def get_landmarks(self, image):
        ...
        for detection in dets:
            face_landmarks = [(item.x, item.y) for item in self.
...
if __name__ == '__main__':
    ...
    parser.add_argument('-i', '--input_imgs_path', type=str, default='imgs',
        help='input images directory path')
    parser.add_argument('-o', '--output_imgs_path', type=str, default=
        'imgs_align', help='output images directory path')
    parser.add_argument('-s', '--output_size', type=int, default=256, help=
    ...
    with tqdm(total=len(files)) as progress:
        ...
        for img_name in files:
            ...
            for i, face_landmarks in enumerate(landmarks_detector.
                get_landmarks(raw_img_path), start=1):
