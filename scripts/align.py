...
def image_align(
    ...
    if shrink > 1:
        rsize = int(np.rint(float(img.size[0]) / shrink)), int(
            np.rint(float(img.size[1]) / shrink)
        )
...
def work_landmark(raw_img_path, img_name, face_landmarks, output_size):
    ...
    if os.path.exists(aligned_face_path):
        return
    image_align(
        raw_img_path, aligned_face_path, face_landmarks, output_size=output_size
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s", "--output_size", type=int, default=256, help="output size of images"
    )
