...
class FaceAlignment:
    def __init__(self, landmarks_type, network_size=NetworkSize.LARGE,
        ...
        face_detector_module = __import__('face_alignment_paddle.detection.' + face_detector,
    ...
    def get_landmarks_from_image(self, image_or_path, detected_faces=None):
        ...
        for i, d in enumerate(detected_faces):
            ...
            inp = paddle.to_tensor(inp.transpose(
                ...
                for i in range(68):
                    if pts[i, 0] > 0 and pts[i, 1] > 0:
                        heatmaps[i] = draw_gaussian(
                            heatmaps[i], pts[i], 2)
                heatmaps = paddle.to_tensor(
                    heatmaps).unsqueeze(0)

                heatmaps = heatmaps.to(self.device)
                depth_pred = self.depth_prediciton_net(
                    paddle.concat((inp, heatmaps), 1)).cpu().reshape((68, 1))
    ...
    def get_landmarks_from_batch(self, image_batch, detected_faces=None):
        ...
        for i, faces in enumerate(detected_faces):
            ...
            for face in faces:
                center = paddle.to_tensor(
                    [(face[2] + face[0]) / 2.0,
                     (face[3] + face[1]) / 2.0], dtype="float32")
                ...
                if self.landmarks_type == LandmarksType._3D:
                    ...
                    heatmaps = heatmaps.to(self.device)
                    depth_pred = self.depth_prediciton_net(
                        paddle.cat((inp, heatmaps), 1)).data.cpu().view(68, 1)
                    pts_img = paddle.cat(
                        (pts_img, depth_pred * (1.0 / (256.0 / (200.0 * scale)))), 1)
                else:
