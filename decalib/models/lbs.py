...
def find_dynamic_lmk_idx_and_bcoords(vertices, pose, dynamic_lmk_faces_idx,
    ...
    aa_pose = paddle.index_select(x=pose.reshape([batch_size, -1, 3]), axis=1,
        index=neck_kin_chain)
    ...
    y_rot_angle = paddle.round(paddle.clip(x=-rot_mat_to_euler(rel_rot_mat) *
        180.0 / np.pi, max=39)).to(dtype='int64')
    neg_mask = y_rot_angle.less_than(y=paddle.to_tensor(0)).to(dtype='int64')
    mask = y_rot_angle.less_than(y=paddle.to_tensor(-39)).to(dtype='int64')
    neg_vals = mask * 78 + (1 - mask) * (39 - y_rot_angle)
    y_rot_angle = neg_mask * neg_vals + (1 - neg_mask) * y_rot_angle
    dyn_lmk_faces_idx = paddle.index_select(x=dynamic_lmk_faces_idx, axis=0,
        index=y_rot_angle)
    dyn_lmk_b_coords = paddle.index_select(x=dynamic_lmk_b_coords, axis=0,
        index=y_rot_angle)
...
def vertices2landmarks(vertices, faces, lmk_faces_idx, lmk_bary_coords):
    ...
    lmk_faces = paddle.index_select(x=faces, axis=0, index=lmk_faces_idx.
        reshape([-1])).reshape([batch_size, -1, 3])
    lmk_faces += paddle.arange(end=batch_size).astype('int64').reshape([-1, 1, 1]
        ) * num_verts
...
def lbs(betas, pose, v_template, shapedirs, posedirs, J_regressor, parents,
    lbs_weights, pose2rot=True, dtype='float32'):
    ...
    if pose2rot:
        ...
        pose_offsets = paddle.matmul(x=pose_feature.reshape([batch_size, -1]), y
            =posedirs).reshape([batch_size, -1, 3])
    ...
    T = paddle.matmul(x=W, y=A.reshape([batch_size, num_joints, 16])).reshape([
        batch_size, -1, 4, 4])
    homogen_coord = paddle.ones(shape=[batch_size, v_posed.shape[1], 1],
        dtype=dtype)
...
def transform_mat(R, t):
    ...
    """
>>>    return paddle.concat(x=[torch.nn.functional.pad(R, [0, 0, 0, 1]), torch
        .nn.functional.pad(t, [0, 0, 0, 1], value=1)], axis=2)
...
def batch_rigid_transform(rot_mats, joints, parents, dtype='float32'):
    ...
    transforms_mat = transform_mat(rot_mats.reshape([-1, 3, 3]), rel_joints.
        reshape(-1, 3, 1)).reshape((-1, joints.shape[1], 4, 4))
    transform_chain = [transforms_mat[:, (0)]]
    for i in range(1, parents.shape[0]):
        curr_res = paddle.matmul(x=transform_chain[parents[i]], y=
            transforms_mat[:, (i)])
...
>>>    rel_transforms = transforms - torch.nn.functional.pad(paddle.matmul(x=
        transforms, y=joints_homogen), [3, 0, 0, 0, 0, 0, 0, 0])
