...
def build_dataloader(config, is_train=True):
    ...
    if 'vox1' in config.training_data:
        data_list.append(VoxelDataset(K=config.K, image_size=config.
            image_size, scale=[config.scale_min, config.scale_max], n_train
            =config.n_train, isSingle=config.isSingle))
    if 'vox2' in config.training_data:
        data_list.append(VoxelDataset(dataname='vox2', K=config.K,
            image_size=config.image_size, scale=[config.scale_min, config.
            scale_max], n_train=config.n_train, isSingle=config.isSingle))
    if 'vggface2' in config.training_data:
        data_list.append(VGGFace2Dataset(K=config.K, image_size=config.
            image_size, scale=[config.scale_min, config.scale_max],
            trans_scale=config.trans_scale, isSingle=config.isSingle))
    if 'vggface2hq' in config.training_data:
        data_list.append(VGGFace2HQDataset(K=config.K, image_size=config.
            image_size, scale=[config.scale_min, config.scale_max],
            trans_scale=config.trans_scale, isSingle=config.isSingle))
    if 'ethnicity' in config.training_data:
        data_list.append(EthnicityDataset(K=config.K, image_size=config.
            image_size, scale=[config.scale_min, config.scale_max],
            trans_scale=config.trans_scale, isSingle=config.isSingle))
    if 'coco' in config.training_data:
        data_list.append(COCODataset(image_size=config.image_size, scale=[
            config.scale_min, config.scale_max], trans_scale=config.
            trans_scale))
    if 'celebahq' in config.training_data:
        data_list.append(CelebAHQDataset(image_size=config.image_size,
            scale=[config.scale_min, config.scale_max], trans_scale=config.
            trans_scale))
    if 'now_eval' in config.training_data:
        data_list.append(NoWVal())
    if 'aflw2000' in config.training_data:
        data_list.append(AFLW2000())
>>>    train_dataset = torch.utils.data.ConcatDataset(data_list)
    if is_train:
        drop_last = True
        shuffle = True
    else:
...
>>>    train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=
        config.batch_size, shuffle=shuffle, num_workers=config.num_workers,
...
class VoxelDataset(paddle.io.Dataset):
    ...
    def __init__(self, K, image_size, scale, trans_scale=0, dataname='vox2',
        n_train=100000, isTemporal=False, isEval=False, isSingle=False):
        self.K = K
        self.image_size = image_size
        if dataname == 'vox1':
            self.kpt_suffix = '.txt'
            self.imagefolder = (
                '/ps/project/face2d3d/VoxCeleb/vox1/dev/images_cropped')
            self.kptfolder = '/ps/scratch/yfeng/Data/VoxCeleb/vox1/landmark_2d'
            self.face_dict = {}
            for person_id in sorted(os.listdir(self.kptfolder)):
                for video_id in os.listdir(os.path.join(self.kptfolder,
                    person_id)):
                    for face_id in os.listdir(os.path.join(self.kptfolder,
                        person_id, video_id)):
                        if 'txt' in face_id:
                            continue
                        key = person_id + '/' + video_id + '/' + face_id
                        name_list = os.listdir(os.path.join(self.kptfolder,
                            person_id, video_id, face_id))
            ...
            self.kptfolder = (
                '/ps/scratch/face2d3d/vox2_best_clips_annotated_torch7')
            self.segfolder = (
                '/ps/scratch/face2d3d/texture_in_the_wild_code/vox2_best_clips_cropped_frames_seg/test_crop_size_400_batch/'
                )
            cleanlist_path = (
                '/ps/scratch/face2d3d/texture_in_the_wild_code/VGGFace2_cleaning_codes/vox2_best_clips_info_max_normal_50_images_loadinglist.npy'
                )
        ...
        n_train = n_train if n_train < len(self.face_list) else len(self.
            face_list)
    ...
    def __getitem__(self, idx):
        ...
        mask_list = []
        if self.isTemporal:
            random_start = np.random.randint(low=0, high=len(name_list) -
                self.K)
            sample_list = range(random_start, random_start + self.K)
        else:
            sample_list = np.array(np.random.randint(low=0, high=len(
                name_list), size=self.K))
        for i in sample_list:
            name = name_list[i]
            image_path = os.path.join(self.imagefolder, person_id, video_id,
                face_id, name + '.png')
            kpt_path = os.path.join(self.kptfolder, person_id, video_id,
                face_id, name + self.kpt_suffix)
            seg_path = os.path.join(self.segfolder, person_id, video_id,
                face_id, name + '.npy')
    ...
    def __getitem__(self, idx):
        while 100:
            ...
            cropped_kpt = np.dot(tform.params, np.hstack([kpt, np.ones([kpt
                .shape[0], 1])]).T).T
            cropped_kpt[:, :2] = cropped_kpt[:, :2] / self.image_size * 2 - 1
            images_array = paddle.to_tensor(data=cropped_image.transpose(2,
                0, 1)).astype('float32')
...
def video2sequence(video_path):
    ...
    imagepath_list = []
    while success:
        imagepath = '{}/{}_frame{:04d}.jpg'.format(videofolder, video_name,
            count)
...
class TestData(paddle.io.Dataset):
    ...
    def __init__(self, testpath, iscrop=True, crop_size=224, scale=1.25,
        face_detector='fan', face_detector_model=None):
        """
        ...
        if isinstance(testpath, list):
            ...
            self.imagepath_list = glob(testpath + '/*.jpg') + glob(testpath +
                '/*.png') + glob(testpath + '/*.bmp')
        elif os.path.isfile(testpath) and testpath[-3:] in ['jpg', 'png', 'bmp'
            ]:
            self.imagepath_list = [testpath]
        elif os.path.isfile(testpath) and testpath[-3:] in ['mp4', 'csv',
            'vid', 'ebm']:
    ...
    def __getitem__(self, index):
        ...
        if self.iscrop:
            if max(h, w) > 1000:
                ...
                image_small = rescale(image, scale_factor, preserve_range=
                    True, multichannel=True)
                detected_faces = self.face_detector.run(image_small.astype(
                    np.uint8))
            else:
            ...
            old_size = (right - left + bottom - top) / 2
            center = np.array([right - (right - left) / 2.0, bottom - (
                bottom - top) / 2.0])
            size = int(old_size * self.scale)
            src_pts = np.array([[center[0] - size / 2, center[1] - size / 2
                ], [center[0] - size / 2, center[1] + size / 2], [center[0] +
                size / 2, center[1] - size / 2]])
        else:
            src_pts = np.array([[0, 0], [0, h - 1], [w - 1, 0]])
        DST_PTS = np.array([[0, 0], [0, self.resolution_inp - 1], [self.
            resolution_inp - 1, 0]])
        tform = estimate_transform('similarity', src_pts, DST_PTS)
        image = image / 255.0
        dst_image = warp(image, tform.inverse, output_shape=(self.
            resolution_inp, self.resolution_inp))
        dst_image = dst_image.transpose(2, 0, 1)
        return {'image': paddle.to_tensor(data=dst_image).astype(dtype=
            'float32'), 'imagename': imagename, 'tform': tform,
            'original_image': paddle.to_tensor(data=image.transpose(2, 0, 1
            )).astype(dtype='float32')}


class EvalData(paddle.io.Dataset):

    def __init__(self, testpath, kptfolder, iscrop=True, crop_size=224,
        scale=1.25, face_detector='fan', face_detector_model=None):
        """
            testpath: folder, imagepath_list, image path, video path
        """
        if isinstance(testpath, list):
