...
def l2_distance(verts1, verts2):
    return paddle.sqrt(x=((verts1 - verts2) ** 2).sum(axis=2)).mean(axis=1
        ).mean()
...
def kl_loss(texcode):
    ...
    KLD_element = mu.pow(y=2).add_(y=paddle.to_tensor(logvar.exp())).scale_(-1
        ).add_(y=paddle.to_tensor(1)).add_(y=paddle.to_tensor(logvar))
...
def albedo_constancy_loss(albedo, alpha=15, weight=1.0):
    ...
    """
    albedo_chromaticity = albedo / (paddle.sum(x=albedo, axis=1, keepdim=
        True) + 1e-06)
    weight_x = paddle.exp(x=-alpha * (albedo_chromaticity[:, :, 1:, :] -
        albedo_chromaticity[:, :, :-1, :]) ** 2).detach()
    weight_y = paddle.exp(x=-alpha * (albedo_chromaticity[:, :, :, 1:] -
        albedo_chromaticity[:, :, :, :-1]) ** 2).detach()
    albedo_const_loss_x = (albedo[:, :, 1:, :] - albedo[:, :, :-1, :]
        ) ** 2 * weight_x
    albedo_const_loss_y = (albedo[:, :, :, 1:] - albedo[:, :, :, :-1]
        ) ** 2 * weight_y
    albedo_constancy_loss = albedo_const_loss_x.mean(
        ) + albedo_const_loss_y.mean()
...
def albedo_ring_loss(texcode, ring_elements, margin, weight=1.0):
    ...
    for i in range(ring_elements - 1):
        for j in range(ring_elements - 1):
            ...
            tot_ring_loss = paddle.add(x=tot_ring_loss, y=paddle.to_tensor(
...
def landmark_loss(predicted_landmarks, landmarks_gt, weight=1.0):
    if paddle.is_tensor(x=landmarks_gt) is not True:
        ...
        real_2d = paddle.concat(x=[landmarks_gt, paddle.ones(shape=(
            landmarks_gt.shape[0], 68, 1))], axis=-1)
    ...
    if ring_type == '51':
        ...
        for i in range(6):
            for j in range(6):
                ...
                tot_ring_loss = paddle.add(x=tot_ring_loss, y=paddle.
                    to_tensor(paddle.nn.functional.relu(x=margin + pd - nd)
                    .mean()))
                count += 1.0
    elif ring_type == '33':
        perm_code = [(0, 1, 3), (0, 1, 4), (0, 1, 5), (0, 2, 3), (0, 2, 4),
            (0, 2, 5), (1, 0, 3), (1, 0, 4), (1, 0, 5), (1, 2, 3), (1, 2, 4
            ), (1, 2, 5), (2, 0, 3), (2, 0, 4), (2, 0, 5), (2, 1, 3), (2, 1,
            4), (2, 1, 5)]
        count = 0.0
        for i in perm_code:
...
def gradient_dif_loss(prediction, gt):
    ...
    diff = paddle.mean(x=(prediction_diff_x - gt_x) ** 2) + paddle.mean(x=(
        prediction_diff_y - gt_y) ** 2)
    return diff.mean()
...
def laplacian_hq_loss(prediction, gt):
    ...
    kernel_size = 3
    kernel = get_laplacian_kernel2d(kernel_size).to(prediction.place).to(
        prediction.dtype)
    kernel = kernel.tile(repeat_times=[c, 1, 1, 1])
    padding = (kernel_size - 1) // 2
    lap_pre = paddle.nn.functional.conv2d(x=prediction, weight=kernel,
        padding=padding, stride=1, groups=c)
    lap_gt = paddle.nn.functional.conv2d(x=gt, weight=kernel, padding=
...
class VGG19FeatLayer(paddle.nn.Layer):
    ...
    def __init__(self):
        ...
        self.mean = paddle.to_tensor(data=[0.485, 0.456, 0.406]).reshape([1, 3,
            1, 1])
...
class IDMRFLoss(paddle.nn.Layer):
    ...
    def patch_extraction(self, featmaps):
        patch_size = 1
        patch_stride = 1
        """Class Method: *.unfold, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
>>>        patches_as_depth_vectors = featmaps.unfold(2, patch_size, patch_stride
            ).unfold(3, patch_size, patch_stride)
        self.patches_OIHW = patches_as_depth_vectors.transpose(perm=[0, 2, 
            3, 1, 4, 5])
        dims = self.patches_OIHW.shape
        self.patches_OIHW = self.patches_OIHW.reshape([-1, dims[3], dims[4],
            dims[5]])
        return self.patches_OIHW

    def compute_relative_distances(self, cdist):
        epsilon = 1e-05
        div = (paddle.min(keepdim=True, x=cdist, axis=1), paddle.argmin(
            keepdim=True, x=cdist, axis=1))[0]
        relative_dist = cdist / (div + epsilon)
        return relative_dist

    def exp_norm_relative_dist(self, relative_dist):
        scaled_dist = relative_dist
        dist_before_norm = paddle.exp(x=(self.bias - scaled_dist) / self.
            nn_stretch_sigma)
    ...
    def mrf_loss(self, gen, tar):
        ...
        gen_feats_norm = paddle.linalg.norm(x=gen_feats, p=2, axis=1,
            keepdim=True)
        tar_feats_norm = paddle.linalg.norm(x=tar_feats, p=2, axis=1,
            keepdim=True)
        ...
        for i in range(BatchSize):
            ...
            cosine_dist_i = paddle.nn.functional.conv2d(x=gen_feat_i,
                weight=patches_OIHW)
        ...
        k_max_nc = (paddle.max(x=rela_dist.reshape([dims_div_mrf[0],
            dims_div_mrf[1], -1]), axis=2), paddle.argmax(x=rela_dist.reshape([
            dims_div_mrf[0], dims_div_mrf[1], -1]), axis=2))[0]
...
class VGG_16(paddle.nn.Layer):
    ...
    def __init__(self):
        ...
        self.conv_1_1 = paddle.nn.Conv2D(in_channels=3, out_channels=64,
            kernel_size=3, stride=1, padding=1)
        self.conv_1_2 = paddle.nn.Conv2D(in_channels=64, out_channels=64,
            kernel_size=3, stride=1, padding=1)
        self.conv_2_1 = paddle.nn.Conv2D(in_channels=64, out_channels=128,
            kernel_size=3, stride=1, padding=1)
        self.conv_2_2 = paddle.nn.Conv2D(in_channels=128, out_channels=128,
            kernel_size=3, stride=1, padding=1)
        self.conv_3_1 = paddle.nn.Conv2D(in_channels=128, out_channels=256,
            kernel_size=3, stride=1, padding=1)
        self.conv_3_2 = paddle.nn.Conv2D(in_channels=256, out_channels=256,
            kernel_size=3, stride=1, padding=1)
        self.conv_3_3 = paddle.nn.Conv2D(in_channels=256, out_channels=256,
            kernel_size=3, stride=1, padding=1)
        self.conv_4_1 = paddle.nn.Conv2D(in_channels=256, out_channels=512,
            kernel_size=3, stride=1, padding=1)
        self.conv_4_2 = paddle.nn.Conv2D(in_channels=512, out_channels=512,
            kernel_size=3, stride=1, padding=1)
        self.conv_4_3 = paddle.nn.Conv2D(in_channels=512, out_channels=512,
            kernel_size=3, stride=1, padding=1)
        self.conv_5_1 = paddle.nn.Conv2D(in_channels=512, out_channels=512,
            kernel_size=3, stride=1, padding=1)
        self.conv_5_2 = paddle.nn.Conv2D(in_channels=512, out_channels=512,
            kernel_size=3, stride=1, padding=1)
        self.conv_5_3 = paddle.nn.Conv2D(in_channels=512, out_channels=512,
            kernel_size=3, stride=1, padding=1)
        ...
        self.mean = paddle.to_tensor(data=np.array([129.1863, 104.7624, 
            93.594]) / 255.0, dtype='float32').astype(dtype='float32').reshape([
            1, 3, 1, 1])
            ...
            if layer.weight is not None:
                if block <= 5:
                    ...
                    if counter > self.block_size[block - 1]:
                        counter = 1
                        block += 1
                    self_layer.weight.data[...] = paddle.to_tensor(data=
                        layer.weight).reshape(self_layer.weight.shape)[...]
                    self_layer.bias.data[...] = paddle.to_tensor(data=layer
                        .bias).reshape(self_layer.bias.shape)[...]
                else:
    ...
    def forward(self, x):
        ...
        """
        out = {}
        """Class Attribute: torch.distributions.Distribution.mean, can not convert, please check whether it is torch.Tensor.*/torch.autograd.function.FunctionCtx.*/torch.distributions.Distribution.* and convert manually"""
>>>        x = x - self.mean
...
class VGGLoss(paddle.nn.Layer):
    ...
    def __init__(self):
        ...
        self.featlayer.load_weights(path=
            'data/face_recognition_model/vgg_face_torch/VGG_FACE.t7')
...
class VGGFace2Loss(paddle.nn.Layer):
    ...
    def reg_features(self, x):
        ...
        x = paddle.nn.functional.interpolate(x=x * 2.0 - 1.0, size=[224, 
            224], mode='bilinear')
...
>>>        feature = feature.reshape([x.shape[0], -1)
    ...
    def transform(self, img):
        img = img[:, ([2, 1, 0]), :, :].transpose(perm=[0, 2, 3, 1]
            ) * 255 - self.mean_bgr
        img = img.transpose(perm=[0, 3, 1, 2])
        return img

    def _cos_metric(self, x1, x2):
        return 1.0 - paddle.nn.functional.cosine_similarity(x1=x1, x2=x2,
            axis=1)
