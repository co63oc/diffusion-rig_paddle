...
def points2bbox(points, points_scale=None):
    ...
    min_coords, _ = paddle.min(x=points, axis=1), paddle.argmin(x=points,
        axis=1)
    xmin, ymin = min_coords[:, (0)], min_coords[:, (1)]
    max_coords, _ = paddle.max(x=points, axis=1), paddle.argmax(x=points,
        axis=1)
...
def augment_bbox(center, bbox_size, scale=[1.0, 1.0], trans_scale=0.0):
    ...
    trans_scale = (paddle.rand(shape=[batch_size, 2]) * 2.0 - 1.0
        ) * trans_scale
    center = center + trans_scale * bbox_size
    scale = paddle.rand(shape=[batch_size, 1]) * (scale[1] - scale[0]) + scale[
        0]
    size = bbox_size * scale
    return center, size


def crop_tensor(image, center, bbox_size, crop_size, interpolation=
    ...
    src_pts = paddle.zeros(shape=[4, 2], dtype=dtype).unsqueeze(axis=0).expand(
    ...
    DST_PTS = paddle.to_tensor(data=[[[0, 0], [crop_size - 1, 0], [
        crop_size - 1, crop_size - 1], [0, crop_size - 1]]], dtype=dtype,
        place=device).expand(shape=[batch_size, -1, -1])
    dst_trans_src = (kornia.geometry.transform.imgwarp.
        get_perspective_transform(src_pts, DST_PTS))
    cropped_image = kornia.geometry.transform.imgwarp.warp_affine(image,
...
class Cropper(object):
    ...
    def crop(self, image, points, points_scale=None):
        ...
        center, bbox_size = augment_bbox(center, bbox_size, scale=self.
            scale, trans_scale=self.trans_scale)
        cropped_image, tform = crop_tensor(image, center, bbox_size, self.
            crop_size)
        return cropped_image, tform
    ...
    def transform_points(self, points, tform, points_scale=None, normalize=True
        ...
        trans_points_2d = paddle.bmm(x=paddle.concat(x=[points_2d, paddle.
            ones(shape=[batch_size, n_points, 1], dtype=points.dtype)],
            axis=-1), y=tform)
        trans_points = paddle.concat(x=[trans_points_2d[:, :, :2], points[:,
            :, 2:]], axis=-1)
        if normalize:
...
def transform_points(points, tform, points_scale=None, out_scale=None):
    ...
    if points_scale:
        ...
        points_2d = (points_2d * 0.5 + 0.5) * points_scale[0]
    batch_size, n_points, _ = points.shape
    trans_points_2d = paddle.bmm(x=paddle.concat(x=[points_2d, paddle.ones(
        shape=[batch_size, n_points, 1], dtype=points.dtype)], axis=-1), y=
        tform)
    if out_scale:
        trans_points_2d[:, :, (0)] = trans_points_2d[:, :, (0)] / out_scale[1
            ] * 2 - 1
        trans_points_2d[:, :, (1)] = trans_points_2d[:, :, (1)] / out_scale[0
            ] * 2 - 1
    trans_points = paddle.concat(x=[trans_points_2d[:, :, :2], points[:, :,
        2:]], axis=-1)
    return trans_points
