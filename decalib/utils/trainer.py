...
>>>            self.writer = torch.utils.tensorboard.SummaryWriter(log_dir=os.
    ...
    def configure_optimizers(self):
        self.opt = paddle.optimizer.Adam(parameters=self.E_flame.parameters(), learning_rate=self.cfg.
...
>>>        masks = masks.view(-1, images.shape[-2], images.shape[-1])
        ...
        opdict = self.deca.decode(codedict, vis_lmk=False, return_vis=False,
            use_detail=False)
        mask_face_eye = paddle.nn.functional.grid_sample(x=self.deca.
            uv_face_eye_mask.expand(shape=[batch_size, -1, -1, -1]), grid=
            opdict['grid'].detach(), align_corners=False)
        predicted_images = opdict['rendered_images'] * mask_face_eye * opdict[
            'alpha_images']
        ...
        if self.cfg.loss.id > 0.0:
            ...
            albedo_images = paddle.nn.functional.grid_sample(x=opdict[
                'albedo'].detach(), grid=opdict['grid'], align_corners=False)
        ...
        if self.cfg.model.jaw_type == 'euler':
            losses['reg_jawpose_roll'] = paddle.sum(x=codedict[
                'euler_jaw_pose'][:, (-1)] ** 2) / 2 * 10.0
            losses['reg_jawpose_close'] = paddle.sum(x=paddle.nn.functional
                .relu(x=-codedict['euler_jaw_pose'][:, (0)]) ** 2) / 2 * 10.0
        all_loss = 0.0
...
"""
                ...
                if self.global_step % self.cfg.train.vis_steps == 0:
                    ...
                    visdict = {'inputs': opdict['images'][visind],
                        ...
                        visind], isScale=True), 'shape_images':
                        shape_images[visind], 'predicted_images': opdict[
                        'predicted_images'][visind]}
