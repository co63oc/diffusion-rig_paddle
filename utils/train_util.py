...
class TrainLoop:
    ...
    def __init__(self, *, model, diffusion, data, batch_size, lr,
        log_interval, save_interval, resume_checkpoint, schedule_sampler=
        None, weight_decay=0.0, stage=1, max_steps=0, auto_scale_grad_clip=1.0
        ):
        self.model = model
        ...
        self.opt = paddle.optimizer.Adam(parameters=self.model.parameters(),
            learning_rate=self.lr, weight_decay=self.weight_decay)
        self.scaler = paddle.amp.GradScaler(incr_every_n_steps=2000,
...
>>>            self.ddp_model = (torch.nn.parallel.distributed.
    ...
    def _load_and_sync_parameters(self):
        ...
        if resume_checkpoint:
            ...
            ckpt = dist_util.load_state_dict(resume_checkpoint,
    ...
    def run_loop(self):
        while (not self.max_steps or self.step + self.resume_step < self.
            max_steps):
            batch = next(self.data)
            image = batch['image']
            rendered, normal, albedo = batch['rendered'], batch['normal'
                ], batch['albedo']
    ...
    def run_step(self, image, physic_cond):
        ...
        paddle.nn.utils.clip_grad_norm_(parameters=self.ddp_model.
            parameters(), max_norm=self.auto_scale_grad_clip)
    ...
    def forward_backward(self, image, physic_cond):
        ...
        t, weights = self.schedule_sampler.sample(image.shape[0], dist_util
            .dev())
        compute_losses = functools.partial(self.diffusion.training_losses,
            self.ddp_model, image, t, model_kwargs={'physic_cond':
            physic_cond, 'x_start': image})
        with paddle.amp.auto_cast():
            losses = compute_losses()
            if isinstance(self.schedule_sampler, LossAwareSampler):
                self.schedule_sampler.update_with_local_losses(t, losses[
                    'loss'].detach())
            loss = (losses['loss'] * weights).mean()
        log_loss_dict(self.diffusion, t, {k: (v * weights) for k, v in
            losses.items()})
        self.scaler.scale(loss).backward()

    def log_step(self):
        logger.logkv('step', self.step + self.resume_step)
        logger.logkv('samples', (self.step + self.resume_step + 1) * self.
            global_batch)
        logger.logkv('time elapsed', str(datetime.timedelta(seconds=time.
        ...
        if self.step_time == 0:
            ...
            self.step_time = 0.1 * self.step_time + 0.9 * (ct - self.
                last_step_time)
            self.last_step_time = ct
        logger.logkv('time est.(10k)', str(datetime.timedelta(seconds=10000 *
            self.step_time)))
        steps_to_go = (self.save_interval - self.step - self.resume_step
            ) % self.save_interval
        logger.logkv('time est.(next ckpt)', str(datetime.timedelta(seconds
            =steps_to_go * self.step_time)))
    ...
    def save(self):
        ...
        def save_checkpoint(rate, params=None):
            ...
            if paddle.distributed.get_rank() == 0:
                ...
                with bf.BlobFile(bf.join(get_blob_logdir(), filename), 'wb'
                    ) as f:
        ...
        if paddle.distributed.get_rank() == 0 and self.stage == 1:
            with bf.BlobFile(bf.join(get_blob_logdir(),
                f'opt{self.step + self.resume_step:06d}.pt'), 'wb') as f:
