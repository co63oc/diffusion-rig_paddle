...
class SpacedDiffusion(GaussianDiffusion):
    ...
    def _wrap_model(self, model):
        if isinstance(model, _WrappedModel):
            return model
        return _WrappedModel(model, self.timestep_map, self.
            rescale_timesteps, self.original_num_steps)
...
class _WrappedModel:
    ...
    def __call__(self, x, ts, **kwargs):
        map_tensor = paddle.to_tensor(data=self.timestep_map, dtype=ts.
            dtype, place=ts.place)
        new_ts = map_tensor[ts]
        if self.rescale_timesteps:
            new_ts = new_ts.astype(dtype='float32') * (1000.0 / self.
                original_num_steps)
