...
class GaussianDiffusion:
    ...
    def __init__(self, *, betas, model_mean_type, model_var_type, loss_type,
        rescale_timesteps=False, p2_weight=False, p2_gamma=1.0, p2_k=1.0):
        ...
        assert (betas > 0).astype('bool').all() and (betas <= 1).astype('bool'
            ).all()
        ...
        self.sqrt_recipm1_alphas_cumprod = np.sqrt(1.0 / self.
            alphas_cumprod - 1)
    ...
    def q_mean_variance(self, x_start, t):
        ...
        log_variance = _extract_into_tensor(self.
            log_one_minus_alphas_cumprod, t, x_start.shape)
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        alpha_t = _extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start
            .shape)
        sigma_t = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod,
            t, x_start.shape)
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        posterior_mean = _extract_into_tensor(self.posterior_mean_coef1, t,
            x_t.shape) * x_start + _extract_into_tensor(self.
            posterior_mean_coef2, t, x_t.shape) * x_t
    ...
    def p_mean_variance(self, model, x, t, clip_denoised=True, denoised_fn=
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output, _ = model(x, self._scale_timesteps(t), **model_kwargs)
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.
            LEARNED_RANGE]:
            assert model_output.shape == (B, C * 2, *x.shape[2:])
            model_output, model_var_values = paddle.split(x=model_output,
                num_or_sections=model_output.shape[1] // C, axis=1)
            ...
            model_variance, model_log_variance = {ModelVarType.FIXED_LARGE:
                (np.append(self.posterior_variance[1], self.betas[1:]), np.
                log(np.append(self.posterior_variance[1], self.betas[1:]))),
                ModelVarType.FIXED_SMALL: (self.posterior_variance, self.
                posterior_log_variance_clipped)}[self.model_var_type]
        ...
        elif self.model_mean_type in [ModelMeanType.START_X, ModelMeanType.
            EPSILON]:
            if self.model_mean_type == ModelMeanType.START_X:
                pred_xstart = process_xstart(model_output)
            else:
                pred_xstart = process_xstart(self._predict_xstart_from_eps(
        ...
        return {'mean': model_mean, 'variance': model_variance,
    ...
    def _predict_xstart_from_eps(self, x_t, t, eps):
        ...
        return _extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x_t.
            shape) * x_t - _extract_into_tensor(self.
            sqrt_recipm1_alphas_cumprod, t, x_t.shape) * eps

    def _predict_xstart_from_xprev(self, x_t, t, xprev):
        assert x_t.shape == xprev.shape
        return _extract_into_tensor(1.0 / self.posterior_mean_coef1, t, x_t
            .shape) * xprev - _extract_into_tensor(self.
            posterior_mean_coef2 / self.posterior_mean_coef1, t, x_t.shape
            ) * x_t

    def _predict_xstart_from_v(self, x_t, t, v):
        assert x_t.shape == v.shape
        alpha_t = _extract_into_tensor(self.sqrt_alphas_cumprod, t, x_t.shape)
        sigma_t = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod,
            t, x_t.shape)
    ...
    def condition_mean(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        ...
        new_mean = p_mean_var['mean'].astype(dtype='float32') + p_mean_var[
            'variance'] * gradient.astype(dtype='float32')
        return new_mean
    ...
    def condition_score(self, cond_fn, p_mean_var, x, t, model_kwargs=None):
        ...
        eps = eps - (1 - alpha_bar).sqrt() * cond_fn(x, self.
            _scale_timesteps(t), **model_kwargs)
        out = p_mean_var.copy()
...
>>>        nonzero_mask = (t != 0).astype(dtype='float32').view(-1, *([1] * (
            len(x.shape) - 1)))
        if cond_fn is not None:
            out['mean'] = self.condition_mean(cond_fn, out, x, t,
                model_kwargs=model_kwargs)
        sample = out['mean'] + nonzero_mask * paddle.exp(x=0.5 * out[
            'log_variance']) * noise
    ...
    def p_sample_loop(self, model, shape, noise=None, clip_denoised=True,
        ...
        """
        final = None
        for sample in self.p_sample_loop_progressive(model, shape, noise=
    ...
    def p_sample_loop_progressive(self, model, shape, noise=None,
        clip_denoised=True, denoised_fn=None, cond_fn=None, model_kwargs=
        None, device=None, progress=False):
        """
        ...
        for i in indices:
            t = paddle.to_tensor(data=[i] * shape[0], place=device)
            with paddle.no_grad():
    ...
    def ddim_sample(self, model, x, t, clip_denoised=True, denoised_fn=None,
        ...
        alpha_bar_prev = _extract_into_tensor(self.alphas_cumprod_prev, t,
            x.shape)
        sigma = eta * paddle.sqrt(x=(1 - alpha_bar_prev) / (1 - alpha_bar)
            ) * paddle.sqrt(x=1 - alpha_bar / alpha_bar_prev)
        noise = paddle.randn(shape=x.shape, dtype=x.dtype)
        mean_pred = out['pred_xstart'] * paddle.sqrt(x=alpha_bar_prev
            ) + paddle.sqrt(x=1 - alpha_bar_prev - sigma ** 2) * eps
        """Class Method: *.view, can not convert, please check whether it is torch.Tensor.*/Optimizer.*/nn.Module.*/torch.distributions.Distribution.*/torch.autograd.function.FunctionCtx.*/torch.profiler.profile.*/torch.autograd.profiler.profile.*, and convert manually"""
>>>        nonzero_mask = (t != 0).astype(dtype='float32').view(-1, *([1] * (
            len(x.shape) - 1)))
    ...
    def ddim_reverse_sample_loop(self, model, x, clip_denoised=True,
        denoised_fn=None, model_kwargs=None, eta=0.0, device=None):
        if device is None:
            device = x.place
        sample_t = []
        xstart_t = []
        ...
        sample = x
        for i in indices:
            t = paddle.to_tensor(data=[i] * len(sample), place=device)
            with paddle.no_grad():
                ...
                T.append(t)
        return {'sample': sample, 'sample_t': sample_t, 'xstart_t':
            xstart_t, 'T': T}

    def ddim_reverse_sample(self, model, x, t, clip_denoised=True,
        ...
        out = self.p_mean_variance(model, x, t, clip_denoised=clip_denoised,
            denoised_fn=denoised_fn, model_kwargs=model_kwargs)
        eps = (_extract_into_tensor(self.sqrt_recip_alphas_cumprod, t, x.
            shape) * x - out['pred_xstart']) / _extract_into_tensor(self.
            sqrt_recipm1_alphas_cumprod, t, x.shape)
        alpha_bar_next = _extract_into_tensor(self.alphas_cumprod_next, t,
            x.shape)
        mean_pred = out['pred_xstart'] * paddle.sqrt(x=alpha_bar_next
    ...
    def ddim_sample_loop_progressive(self, model, shape, noise=None,
        clip_denoised=True, denoised_fn=None, cond_fn=None, model_kwargs=
        None, device=None, progress=False, eta=0.0):
        """
    ...
    def _vb_terms_bpd(self, model, x_start, x_t, t, clip_denoised=True,
        ...
        """
        true_mean, _, true_log_variance_clipped = (self.
            q_posterior_mean_variance(x_start=x_start, x_t=x_t, t=t))
        out = self.p_mean_variance(model, x_t, t, clip_denoised=
            clip_denoised, model_kwargs=model_kwargs)
        kl = normal_kl(true_mean, true_log_variance_clipped, out['mean'],
            out['log_variance'])
        kl = mean_flat(kl) / np.log(2.0)
        decoder_nll = -discretized_gaussian_log_likelihood(x_start, means=
            out['mean'], log_scales=0.5 * out['log_variance'])
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None
        ...
        terms = {}
        model_output, losses = model(x_t, self._scale_timesteps(t), **
            model_kwargs)
        if self.model_mean_type == ModelMeanType.EPSILON:
            target = noise
        else:
            raise NotImplementedError(
                f'unknown model mean type {self.model_mean_type}')
        assert model_output.shape == target.shape
        if self.p2_weight:
            weight = _extract_into_tensor(1 / (self.p2_k + self.snr) **
                self.p2_gamma, t, target.shape)
            terms['mse'] = mean_flat(weight * (target - model_output) ** 2)
        else:
            terms['mse'] = mean_flat((target - model_output) ** 2)
        terms['loss'] = terms['mse']
        for key in losses:
            terms[key] = losses[key]
            terms['loss'] = terms['loss'] + losses[key]
        return terms
    ...
    def _prior_bpd(self, x_start):
        ...
        t = paddle.to_tensor(data=[self.num_timesteps - 1] * batch_size,
            place=x_start.place)
        qt_mean, _, qt_log_variance = self.q_mean_variance(x_start, t)
        kl_prior = normal_kl(mean1=qt_mean, logvar1=qt_log_variance, mean2=
            0.0, logvar2=0.0)
        ...
        for t in list(range(self.num_timesteps))[::-1]:
            ...
            with paddle.no_grad():
                out = self._vb_terms_bpd(model, x_start=x_start, x_t=x_t, t
                    =t_batch, clip_denoised=clip_denoised, model_kwargs=
                    model_kwargs)
            vb.append(out['output'])
            xstart_mse.append(mean_flat((out['pred_xstart'] - x_start) ** 2))
            eps = self._predict_eps_from_xstart(x_t, t_batch, out[
                'pred_xstart'])
            mse.append(mean_flat((eps - noise) ** 2))
        vb = paddle.stack(x=vb, axis=1)
        xstart_mse = paddle.stack(x=xstart_mse, axis=1)
        mse = paddle.stack(x=mse, axis=1)
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    ...
    """
    res = paddle.to_tensor(data=arr).to(device=timesteps.place)[timesteps
