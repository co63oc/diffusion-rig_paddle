...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if self.model_var_type in [ModelVarType.LEARNED, ModelVarType.LEARNED_RANGE]:
            ...
            model_output, model_var_values = paddle_add.split(
                x=model_output, num_or_sections=model_output.shape[1] // C, axis=1
            )
    ...
    def _predict_xstart_from_v(self, x_t, t, v):
        ...
        sigma_t = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        terms = {}
        model_output, losses = model(x_t, self._scale_timesteps(t), **model_kwargs)
        if self.model_mean_type == ModelMeanType.EPSILON:
            target = noise
        else:
            raise NotImplementedError(f"unknown model mean type {self.model_mean_type}")
    ...
    def _prior_bpd(self, x_start):
        ...
        t = paddle.to_tensor(
            data=[self.num_timesteps - 1] * batch_size, place=x_start.place
        )
...
def _extract_into_tensor(arr, timesteps, broadcast_shape):
    ...
    """
    res = paddle.to_tensor(data=arr, place=timesteps.place)[timesteps].astype(
        dtype="float32"
    )
